...
import jp.ac.it_college.s20015.quiz.databinding.FragmentQuizBinding

class QuizFragment : Fragment() {
    companion object {
        const val MAX_COUNT = 10                //問題数
    ...
    private var totalElapsedTime = 0L //合計した解答時間
    private var correctCount = 0      //問題番号
    ...
    private val onChoiceClick = View.OnClickListener { v ->
        ...
        if (v.text == quizList[current].choices[0]) { //choicesの部分は変更アリ
            // 正解パターン
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.choiceE.setOnClickListener(onChoiceClick)
        binding.choiceF.setOnClickListener(onChoiceClick)
        next()
    }
    ...
    private fun isBulkEnableButton(flag: Boolean) {
        ...
        binding.choiceE.isEnabled = flag
        binding.choiceF.isEnabled = flag
    }
    ...
    private fun setQuiz(position: Int) {
        ...
        binding.choiceE.text = randomChoice[4]
        binding.choiceF.text = randomChoice[5]
    }
}